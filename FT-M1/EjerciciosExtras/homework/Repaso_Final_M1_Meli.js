...
var countProps = function (obj) {
  ...
  for (let propiedad in obj) {
    //itera sobre todas las propiedades de un objeto   el for of es para array
    counter++; //si entro necesitamos aumentar el contador
    if (typeof obj[propiedad] === "object") {
      //accedemos al valor, como entramos al indice del objeto        typeof(arr)--> 'object'
      if (!Array.isArray(obj[propiedad])) {
        //Si no es un array donde estamos parados
...
LinkedList.prototype.changeNotNumbers = function () {
  ...
  while (current) {
    ...
    if (isNaN(current.value)) {
      //devuelve un booleano, verifica si es numero o no, da false cuando es un numero. Ej: ¿1 no es un numero? Si hay un numero isNaN da false
